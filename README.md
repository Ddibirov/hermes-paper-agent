...
A working end-to-end example of *your own nightly newspaper* — written overnight
from your calendar, budget, feeds, notes, and a couple of live data feeds, and
printed the way a real broadsheet is. This repo is the **generator**: it
assembles tomorrow's edition as a folder of markdown articles. The reading and
printing is done by the [Vael Paper](https://github.com/vaelkeep/vael-paper)
engine, which this generator feeds.

The whole thing is an agent. You talk to Hermes once and a scheduled run writes,
...
- [Step 2 — Install Vael Paper (the engine that prints it)](#step-2--install-vael-paper-the-engine-that-prints-it)
- [Step 3 — Install the agent (this example)](#step-3--install-the-agent-this-example)
- [Step 4 — Make the paper yours](#step-4--make-the-paper-yours)
- [Step 5 — Add your own dynamic content](#step-5--add-your-own-dynamic-content)
...
| **The agent** | [Hermes Agent](https://hermes-agent.nousresearch.com) runs the desks, checks the result, fixes what's wrong, and schedules the whole run. Hermes is the "night editor". |

You own everything. `editions/paper.json` sets the masthead and which sections
the paper has; `inbox/` is your material; `scripts/` are your data desks. The
example ships with fiction fixtures, so run it once unchanged to see a full
paper, then replace the standing parts with your real ones.
...
- **Prose desks** are the model (you, as Hermes). They take a handful of
...
keyless APIs (Open-Meteo, Yahoo Finance).

---

## Step 1 — Install the required software

**Python + uv** (macOS / Linux):
...
Then complete the one-time setup and pick a model:

```bash
hermes setup      # the setup wizard
...
docs; you can switch models any time with `hermes model`.

## Step 2 — Install Vael Paper (the engine that prints it)

The reader/engine is a separate project. Clone it, build the reader once, and
start the server:
...
Point the server at **this repo's** `editions/` so it prints *your* paper, not
the demo:
...
Open http://localhost:8791 — you are reading the sample edition. Turn pages
with the arrow keys; press **Source** to see the markdown behind any page.

## Step 3 — Install the agent (this example)

Clone this repo wherever you keep it (the agent's working directory):
...
## Step 4 — Make the paper yours

The paper's identity lives in one file, `editions/paper.json` — the masthead,
the motto, the date you started, and the section order. A generator never
writes it: you do, once, by hand. Edit it to your name and your sections:
...
Then replace the standing fixtures in `inbox/` with your real material. The
shape of each is in `inbox/README.md`, but in short:
...
inbox/
  ...
  photos/           a photo library, by filename
```
...
## Step 5 — Add your own dynamic content
...
so it is right every night. The `weather-desk.py` and `finance-desk.py`
scripts are working templates for exactly this pattern:
...
2. **Render** frontmatter + markdown into `editions/<date>/articles/<nn>.md`,
   matching the fields the engine forgives (`headline`, `deck`, `section`,
...
Then tell Hermes to run it — the agent can call the script through the write →
check → fix loop (see [Sample prompts](#sample-prompts)). Two stock desks
worth copying for the shape: `weather-desk.py` (live API → chart article) and
`finance-desk.py` (live API → quote table, tickers as arguments).

**Live feeds in this example**

| Desk | Source | Default data | Config |
|---|---|---|---|
| `weather-desk.py` | Open-Meteo (free, no key) | Washington DC forecast | `--lat/--lon/--place` or `WEATHER_LAT/WEATHER_LON/WEATHER_PLACE` |
...
directory (so `AGENTS.md` loads). Here are the ones you'll use, in order.

**See the sample paper (no real data needed):**
...
[vael-paper-write]`, schedule `every day at 4am`, and the prompt becomes:
...
Your paper is on the front porch (well: at http://localhost:8791) at breakfast,
and if a desk gave up it is visible as a printer's mark in the reader — not a
blank page, and not a fatal failure at 4 a.m.

---

## License

MIT — do what you like, keep the notice. The paper you generate is yours. The
`inbox/` fixtures and the lead-desk sample are fiction, written to match the
Vael Paper demo's shapes, and keep its fiction note. The Vael Paper engine has
its own MIT license — see its `LICENSE` and `NOTICE`.