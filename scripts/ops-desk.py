...
def build_article(checks: dict) -> str:
    ...
    body = {
        "live": "Проверка кодом, без сети и ключей: шлюз принимает "
                ...
                "чтение перед печатью. Код снял всё за секунды и ничего не "
                "выдумал — каждый статус в таблице живая проверка, а не "
                "предположение, и утром читать её можно не глядя на время.",
